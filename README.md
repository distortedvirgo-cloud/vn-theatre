...
С v1.6.2 эмоция меняется **внутри одного ответа**: ответ играется реплика за
репликой, и модель может поставить промежуточный тег
`<<scene: expression=ID>>` на строке, где настроение меняется — спрайт
переключается, когда игрок доходит до этой строки (хвостовой тег, как и
раньше, задаёт эмоцию всему ответу).
...
### Скорость генерации (v1.6.3)
Генерация по умолчанию — **10 шагов** (≈9 с на RTX 5060 Ti вместо ~33 с при
30 шагах; качество близко к 30 — линии чистые, у 8 шагов заметно грязнее
волосы). Шаги меняются в настройках (**Steps**). Сам ComfyUI должен быть
запущен с флагами быстрых весов, иначе движок стримит веса из RAM каждый шаг:

```
python_embeded\python.exe main.py --listen 127.0.0.1 --port 8188 ^
  --enable-cors-header * --fast --highvram
```

`--highvram` держит модели в VRAM (не перезагружает с диска), `--fast`
включает fp16-аккумуляцию — оба почти не влияют на качество. Прозрачный
фон спрайтов (RMBG) стоит ~0.6 с.
